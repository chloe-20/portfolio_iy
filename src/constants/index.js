import work01 from "../assets/img/port07.jpg";
import work02 from "../assets/img/port01.jpg";
import work03 from "../assets/img/port02.jpg";
import work04 from "../assets/img/port03.jpg";
import work05 from "../assets/img/port04.jpg";
import work06 from "../assets/img/port05.jpg";
import work07 from "../assets/img/port06.jpg";

export const headerNav = [
  {
    title: "INTRO",
    url: "#intro",
  },
  {
    title: "ABOUT ME",
    url: "#aboutMe",
  },
  {
    title: "EXPERIENCE",
    url: "#experience",
  },
  {
    title: "WORK",
    url: "#work",
  },
  {
    title: "CONTACT",
    url: "#contact",
  },
];

export const introText = {
  title: ["I AM", "MARKUP DEVELOPER"],
  desc: ["SLOW AND STEADY", "JUST KEEP GOING"],
};

export const experienceText = [
  {
    period: "2021.04 - 2021.09",
    text: ["멀티미디어콘텐츠 제작(웹&영상)", "방송정보국제교육원(IIBI)"],
    title: "멀티미디어콘텐츠(웹&영상) 제작 교육 과정 수료",
    info: ["PERIOD : 6 MONTHS", "USE STACK : HTML5/CSS3, JAVASCRIPT, JQUERY, PHOTOSHOP, ILLUSTRATOR, PREMIERE PRO, AFTER EFFECTS, CINEMA 4D"],
  },
  {
    period: "2021.09 - 2023.08",
    text: ["웹 에이전시", "UI연구개발부, 주임"],
    title: "운영, 구축, 개선 등 다양한 프로젝트 수행",
    info: ["PERIOD : 2 YEARS", "USE STACK : HTML5/CSS3, JAVASCRIPT, JQUERY, PHOTOSHOP, FIGMA, ZEPLIN, SLACK"],
  },
  {
    period: "2023.11 - 2024.05",
    text: ["웹 에이전시", "프리랜서 퍼블리셔"],
    title: "케이뱅크 접근성 개선 프로젝트 수행(기업뱅킹 담당)",
    info: ["PERIOD : 7 MONTHS", "USE STACK : HTML5/CSS3, JAVASCRIPT, JQUERY"],
  },
];

export const workText = [
  {
    num: "01",
    title: "케이뱅크 접근성 개선 프로젝트",
    desc: "운영되고 있는 케이뱅크 기업뱅킹 웹/앱 페이지에 접근성 속성을 반영하여 접근성을 개선하였습니다.",
    img: work01,
    view: "https://biz.kbanknow.com/ib20/mnu/CBKCSC020100",
    name: "케이뱅크 기업뱅킹 웹/앱",
  },
  {
    num: "02",
    title: "포트폴리오 사이트(2023)",
    desc: "개인 포트폴리오 사이트 2023 리액트 버전입니다.",
    img: work02,
    view: "https://github.com/chloe-20/portfolio_iy",
    name: "2023 리액트로 만든 포트폴리오",
  },
  {
    num: "03",
    title: "하나원큐 회원가입개편",
    desc: "HTML, CSS, Javascript, JQuery를 활용하여 페이지 구축했습니다.",
    img: work03,
    view: "https://www.hanabank.com/oneQp/index.html#none",
    name: "하나원큐 앱",
  },
  {
    num: "04",
    title: "한경 아르테 구축",
    desc: "HTML, CSS, Javascript, JQuery를 활용하여 페이지 구축했습니다.",
    img: work04,
    view: "https://www.arte.co.kr/",
    name: "한경 아르떼 웹 사이트",
  },
  {
    num: "05",
    title: "삼성 언팩 2022",
    desc: "AEM 프로그램을 활용하여 국가별 출시 예정인 제품 관련 데이터 입력 및 배포하였습니다.",
    img: work05,
    view: "https://www.samsung.com/sec/smartphones/galaxy-s23/",
    name: "삼성 갤럭시 S23 웹 사이트",
  },
  {
    num: "06",
    title: "LG CNS MOP 광고플랫폼 구축",
    desc: "HTML, CSS, Javascript, JQuery를 활용하여 페이지 구축했습니다.",
    img: work06,
    view: "https://mop.co.kr/intro-page/index.html",
    name: "LG CNS MOP 광고플랫폼 웹 사이트",
  },
  {
    num: "07",
    title: "MG기업뱅킹(인터넷/스마트) 구축",
    desc: "HTML, CSS, Javascript, JQuery를 활용하여 페이지 구축했습니다.",
    img: work07,
    view: "https://biz.kfcc.co.kr/",
    name: "MG기업뱅킹 웹 사이트",
  },
];

export const contactText = [
  {
    link: "https://github.com/chloe-20",
    title: "GITHUB.COM/CHLOE-20",
  },
  {
    link: "mailto:ioiuioiciy88@gmail.com",
    title: "IOIUIOICIY88@GMAIL.COM",
  },
];
